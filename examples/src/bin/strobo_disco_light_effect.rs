...
fn main() {
    ...
    // strobo effekt
    for _ in 0..64 {
        white_display_bytes.extend(encode_rgb(255, 255, 255));
    }

    let mut empty_display_bytes = vec![];
    for _ in 0..64 {
        empty_display_bytes.extend(encode_rgb(0, 0, 0));
    }

    loop {
        spi.write_all(&white_display_bytes);
        sleep_busy_waiting(1);
        spi.write_all(&empty_display_bytes);
